...
def parStandards(request, standard):
    return views.parseStandardsXml(request, resource_filename('mwsu_curriculum', 'standards/'+standard+'.xml'))
...
def parTeachingAssignment(request, schedule):
    return views.scheduleTeachingAssignmentHtml(request, resource_filename('mwsu_curriculum', 'schedules/'+schedule+'.xml'))
